...
public class correctEFDT extends AbstractClassifier implements MultiClassClassifier {
    ...
    public static double computeHoeffdingBound(double range, double confidence, double n) {
        return 2 * Math.sqrt(((range * range) * Math.log(1.0 / confidence))
                / (2.0 * n));
    }
            ...
            if (bestSplitSuggestions.length < 2) {
                ...
                if (shouldSplit) {
                    for (Integer i : node.usedNominalAttributes) {
                        if (bestSuggestion.splitTest != null && bestSuggestion.splitTest.getAttsTestDependsOn()[0] == i) {
                            shouldSplit = false;
                            break;
                        }
                    }
    ...
    public interface EFDTNode {
        ...
        void learnFromInstance(Instance inst, correctEFDT ht, EFDTSplitNode parent, int parentBranch);
    ...
    public static class Node extends AbstractMOAObject {
        ...
        public double[] getClassVotes(Instance inst, correctEFDT ht) {
            return this.observedClassDistribution.getArrayCopy();
        }
        ...
        public void describeSubtree(correctEFDT ht, StringBuilder out,
                                    int indent) {
    ...
    public class EFDTSplitNode extends SplitNode implements EFDTNode {
        ...
        public void killSubtree(correctEFDT ht) {
            for (Node child : this.children) {
                if (child != null) {
        ...
        public AttributeSplitSuggestion[] getBestSplitSuggestions(
                SplitCriterion criterion, correctEFDT ht) {
        ...
        @Override
        public void learnFromInstance(Instance inst, correctEFDT ht, EFDTSplitNode parent, int parentBranch) {
        ...
        protected void reEvaluateBestSplit(EFDTSplitNode node, EFDTSplitNode parent,
            ...
            SplitCriterion splitCriterion = (SplitCriterion) getPreparedClassOption(correctEFDT.this.splitCriterionOption);
            double hoeffdingBound = computeHoeffdingBound(splitCriterion.getRangeOfMerit(node.getClassDistributionAtTimeOfCreation()),
                    correctEFDT.this.splitConfidenceOption.getValue(), node.observedClassDistribution.sumOfValues());

            // get best split suggestions
            AttributeSplitSuggestion[] bestSplitSuggestions = node.getBestSplitSuggestions(splitCriterion, correctEFDT.this);
            Arrays.sort(bestSplitSuggestions);
            ...
            double tieThreshold = correctEFDT.this.tieThresholdOption.getValue();
            ...
            if (deltaG > hoeffdingBound
                ...
                if (splitDecision.splitTest == null) {
                    ...
                    node.killSubtree(correctEFDT.this);
                    ...
                    if (node.splitTest == splitDecision.splitTest
                        ...
                        this.killSubtree(correctEFDT.this);
                        ...
                        correctEFDT.this.activeLeafNodeCount--;
                        correctEFDT.this.decisionNodeCount++;
                        correctEFDT.this.activeLeafNodeCount += splitDecision.numSplits();
                    ...
                    if (parent == null) {
                        ...
                        correctEFDT.this.treeRoot = newSplit;
                    } else {
    ...
    public static abstract class LearningNode extends Node {
        ...
        public abstract void learnFromInstance(Instance inst, correctEFDT ht);
    }
    ...
    public class EFDTLearningNode extends LearningNodeNBAdaptive implements EFDTNode {
        ...
        @Override
        public void learnFromInstance(Instance inst, correctEFDT ht) {
    ...
    static class VFDT extends correctEFDT {
