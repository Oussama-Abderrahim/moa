 ...
 * @version $Revision: 7 $
 */
public class MisclassificationSplitCriterion extends AbstractOptionHandler implements
        SplitCriterion {
    ...
    public static double computeMisclassificationError(double[] dist, double distSumOfWeights) {
        double max = 0;
        for (int i = 0; i < dist.length; i++) {
            double relFreq = dist[i] / distSumOfWeights;
            if (relFreq > max) max = relFreq;
        }
        return 1 - max;
    }

    public static double computeMisclassificationError(double[] dist) {
        return computeMisclassificationError(dist, Utils.sum(dist));
    }
